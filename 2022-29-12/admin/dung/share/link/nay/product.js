            ...
            for (var i = 0; i <= data.length; i++) {
                if (data[i] != null) {
                    listProduct += "<tr>" +
                                    ...
                                    "<td>" +
                                        "<a href='product_update.html?id=" + data[i].id + "' style='color: black; border: 1px solid black; padding: 4px; font-size: 16px; margin-top: 10px;'>UPDATE</a><br><br>" +
...
function addProduct(e) {
    ...
    fetch('http://103.173.154.17:8080/vintage/product/upload', {
        method: 'POST',
        headers: {
...
function myFunction(id) {
    ...
    if (confirm(text) == true) {
        fetch('http://103.173.154.17:8080/vintage/product/delete/' + id, {
            method: 'DELETE',
            })
            .then(response => {window.location.href="product.html";});
    }
}